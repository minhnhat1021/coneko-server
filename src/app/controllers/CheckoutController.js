...
class RoomsController {
    ...
    payPalCheckout(req, res, next) {
        const { totalPrice,roomId, userId, roomName } = req.body
    ...
    confirmPayPalCheckout = (req, res) => {
        const {totalPrice, paymentId, payerId } = req.body;
        ...
        paypal.payment.execute(paymentId, execute_payment_json, function (error, payment) {
            if (error) {
                console.error(error);
                res.status(500).json({ error: 'Error confirming PayPal payment' });
            } else {
                console.log("Payment confirmed:", JSON.stringify(payment));
                res.json({data: { message: 'Payment successful', payment }});
            }
        });
    }
}
